...
`);
     ...
     */
    parseProjects(projectsArg, fileArg) {
    ...
    readProjectsFromFile(filePath) {
        try {
            ...
            throw new ValidationError(`Failed to read projects file: ${filePath}. ${error.message}`);
        }
    }
    /**
     * Parse and validate numeric options
     */
    parseNumber(value, optionName) {
        if (!value)
            return undefined;
    ...
    validateArgs(args) {
        ...
        if (!args.projects || args.projects.length === 0) {
            throw new ValidationError('No projects specified. Use --projects or --file option to specify project numbers.');
        }
