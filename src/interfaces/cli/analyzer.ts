...
class ESFAnalyzer {
  ...
  private analyzer: NetworkAnalyzer | null = null;
  ...
  async run(): Promise<void> {
    try {
      ...
      console.error('❌ Analyzer failed:', (error as Error).message);
      process.exit(1);
    }
  }
   ...
   */
  private async startRecording(projectNumber?: string): Promise<void> {
    if (!this.analyzer) {
  ...
  private async stopRecording(): Promise<void> {
    ...
    const pdfRequests = session.requests.filter(r => 
   ...
   * Add manual marker
   */
  private async addMarker(description: string): Promise<void> {
    if (!this.analyzer) {
      console.log('❌ Analyzer not initialized');
      return;
    }
