...
export class ChromeTabManager {
  constructor(private chromePort: number = 9222) {}
  ...
  async listTabs(): Promise<ChromeTab[]> {
    try {
      ...
      const data = await response.json() as ChromeTab[];
      return data.filter((tab: ChromeTab) => tab.type === 'page');
    } catch (error) {
      logger.error(`Failed to list Chrome tabs: ${(error as Error).message}`);
   ...
   */
  async findESFTab(): Promise<ChromeTab | null> {
    const tabs = await this.listTabs();
    
    // Look for ESF or identita.gov.cz tabs
    const esfTab = tabs.find(tab => 
      tab.url.includes('esf') || 
   ...
   */
  async findActiveTab(): Promise<ChromeTab | null> {
    const tabs = await this.listTabs();
   ...
   */
  async logAvailableTabs(): Promise<void> {
    try {
      const tabs = await this.listTabs();
      ...
      logger.error(`Failed to log available tabs: ${(error as Error).message}`);
    }
  }
}